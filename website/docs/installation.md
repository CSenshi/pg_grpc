...
Three supported paths, ordered universal → fastest → containerized.

## Prerequisites

- PostgreSQL **13–18** with server development headers
- The right path for your platform (below)
...
## Path B — Pre-built `.deb` from GitHub releases
...
Wrap path B on top of the official Postgres image. Drop this `Dockerfile` next to your `docker-compose.yml`:
...
To auto-create the extension on first start, add an init script:

```bash
mkdir -p docker-initdb
cat > docker-initdb/01-pg_grpc.sql <<'SQL'
